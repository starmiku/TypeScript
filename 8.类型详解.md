...
var person2: PersonInterface = { name: "Bob", ag
...
logPoint({ x: 10, y: 20 });                // ✅ 直接字面量
var point3D = { x: 5, y: 5, z: 5 };
logPoint(point3D);                          // ✅ 多出的 z 会被忽略（多余的属性是允许的）
```
...
register({ name: "Alice" });              // ✅ age 可省略
register({ name: "Bob", age: 30 });       // ✅ 带 age 也可以
```
